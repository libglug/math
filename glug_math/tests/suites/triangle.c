...
{
    struct glug_triangle t = { { -0.5f, -0.5f, 0.25f }, { 0.5f, -0.5f, 0.5f }, { 0.f, 0.5f, 0.75f } };
    struct glug_vec3 bary = { 1.f, 0.f, 0.f };
    struct glug_vec3 exp = t.a;

    struct glug_vec3 p = glug_triangle_barycentric(&t, &bary);
    ...
    bary.z = 0.f;
    exp = t.b;
    p = glug_triangle_barycentric(&t, &bary);
    ASSERT_VEC3_EQUAL(&p, &exp, 0.f);
...
{
    struct glug_triangle t = { { -1.f, 1.f, 0.5f }, { -1.f, 2.f, -0.5f }, { -2.f, 1.f, 0.5f } };
    struct glug_vec3 tril = { 2.f, 2.f, 2.f };
    struct glug_vec3 exp = { -1.34108f, 1.24118f, 0.25881f };

    struct glug_vec3 p = glug_triangle_trilinear(&t, &tril);
    ...
    p = glug_triangle_trilinear(&t, &tril);
    ASSERT_VEC3_EQUAL(&p, &exp, 0.00001f);
}
...
static void test_to_barycentric(void)
{
    ...
    exp.y = 1.f;
    exp.z = 0.f;
    bary = glug_triangle_to_barycentric(&t, &p);
    ASSERT_VEC3_EQUAL(&bary, &exp, 0.f);
...
static void test_to_trilinear(void)
{
    ...
    struct glug_vec3 tril = glug_triangle_to_trilinear(&t, &p);
...
{
    struct glug_triangle t = { { -1.f, 0.f, -1.f }, { 1.f, 0.f, -1.f }, { -1.f, 0.f, 1.f } };
    struct glug_vec3 exp = { 0.f, -1.f, 0.f };
    struct glug_vec3 n = glug_triangle_normal(&t);
...
{
    struct glug_triangle t = { { -1.f, 1.f, 1.f }, { -1.f, 2.f, 1.f }, { -2.f, 1.f, -1.f } };

    struct glug_vec3 c = { 1.f, 1.f, 1.f };
    struct glug_vec3 inc = glug_triangle_trilinear(&t, &c);
    glug_bool_t contains = glug_triangle_contains_point(&t, &inc);
    CU_ASSERT_TRUE(contains);

    struct glug_vec3 p = { -1.17588f, 1.39328f, 0.64823f };
    contains = glug_triangle_contains_point(&t, &p);
    CU_ASSERT_TRUE(contains);
...
static void test_distance(void)
{
    ...
    float exp = 1.f;
    float dist = glug_triangle_distance_to_point(&t, &p);
    ...
    p.z = 0.64823f;
    exp = 0.f;
    dist = glug_triangle_distance_to_point(&t, &p);
    CU_ASSERT_DOUBLE_EQUAL(dist, exp, 0.f);
}
...
static void test_project_point(void)
{
    ...
    glug_triangle_project_point(&t, &dst);
    ASSERT_VEC3_EQUAL(&dst, &exp, 0.f);
}

int main(void)
{
    CU_pSuite triangle_suite = create_suite("triangle", NULL, NULL);
    if (!triangle_suite) return CU_get_error();

    ADD_TEST(triangle_suite, equal);
    ADD_TEST(triangle_suite, from_barycentric);
    ADD_TEST(triangle_suite, from_trilinear);
    ADD_TEST(triangle_suite, to_barycentric);
    ADD_TEST(triangle_suite, to_trilinear);
